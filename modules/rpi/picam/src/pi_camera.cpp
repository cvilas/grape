...
auto toSDLPixelFormat(const libcamera::PixelFormat& fmt) -> SDL_PixelFormat {
  // TODO(Vilas): Add MJPEG and NV21 here
  // Map common libcamera formats to SDL3 formats
  if (fmt == libcamera::formats::RGB888) {
    return SDL_PIXELFORMAT_RGB24;
  }
  if (fmt == libcamera::formats::BGR888) {
    return SDL_PIXELFORMAT_BGR24;
  }
  if (fmt == libcamera::formats::ARGB8888) {
    return SDL_PIXELFORMAT_ARGB8888;
  }
  if (fmt == libcamera::formats::ABGR8888) {
    return SDL_PIXELFORMAT_ABGR8888;
  }
  if (fmt == libcamera::formats::YUYV) {
    return SDL_PIXELFORMAT_YUY2;
  }
  if (fmt == libcamera::formats::UYVY) {
    return SDL_PIXELFORMAT_UYVY;
  }
  // For other formats, return a generic format (SDL will try to handle it)
  // Note: May require conversion in the display pipeline
...
struct PiCamera::Impl {
  ...
  std::atomic<bool> camera_started{ false };
...
void PiCamera::Impl::configureStream() {
  //TODO(Vilas): Understand this method properly - compare with hello
  ...
  // Try to configure for RGB888 (24-bit RGB) for compatibility with SDL3
  // If not available, try YUYV which SDL supports
  // If neither available, let libcamera pick the best format
  const auto& rgb888_sizes = stream_config.formats().sizes(libcamera::formats::RGB888);
  const auto& yuyv_sizes = stream_config.formats().sizes(libcamera::formats::YUYV);

  if (!rgb888_sizes.empty()) {
    stream_config.pixelFormat = libcamera::formats::RGB888;
    // Select highest resolution for RGB888
    auto best_size = rgb888_sizes[0];
    for (const auto& size : rgb888_sizes) {
      if (size.width * size.height > best_size.width * best_size.height) {
        best_size = size;
      }
    }
    stream_config.size = best_size;
    syslog::Info("Requesting format: {}x{}, RGB888", best_size.width, best_size.height);
  } else if (!yuyv_sizes.empty()) {
    stream_config.pixelFormat = libcamera::formats::YUYV;
    // Select highest resolution for YUYV
    auto best_size = yuyv_sizes[0];
    for (const auto& size : yuyv_sizes) {
      if (size.width * size.height > best_size.width * best_size.height) {
        best_size = size;
      }
    }
    stream_config.size = best_size;
    syslog::Info("Requesting format: {}x{}, YUYV", best_size.width, best_size.height);
  } else {
    // Neither RGB888 nor YUYV available, let libcamera choose the best available format
    syslog::Warn("Neither RGB888 nor YUYV supported, using camera's default format");
  }
...
void PiCamera::Impl::processRequest(libcamera::Request* request) {
  ...
  libcamera::Request* old = latest_request.exchange(request, std::memory_order_release);
...
void PiCamera::acquire() {
  ...
  const auto frame =
      camera::ImageFrame{ .header = { .pitch = static_cast<std::uint32_t>(stream_config.stride),
