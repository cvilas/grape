...
struct LogFormatter {
  static auto format(const grape::log::Record& record) -> std::string {
    return std::format("[{:9s}] [{}] {}", toString(record.severity), record.timestamp,
                       record.message.cStr());
  }
};
...
auto SDL_AppInit(void** appstate, int /*argc*/, char** /*argv*/) -> SDL_AppResult {
  ...
  *appstate = app_state_ptr.get();
  ...
  auto cameras_ids = std::unique_ptr<SDL_CameraID, decltype(camera_ids_deleter)>(
      SDL_GetCameras(&app_state_ptr->camera_count), camera_ids_deleter);
  if ((cameras_ids == nullptr) or (app_state_ptr->camera_count == 0)) {
    grape::syslog::Critical("No cameras found, or unable to enumerate cameras: {}", SDL_GetError());
    return SDL_APP_FAILURE;
  }

  grape::syslog::Info("Found {} camera{}", app_state_ptr->camera_count,
                      app_state_ptr->camera_count > 1 ? "s" : "");
  for (int i = 0; i < app_state_ptr->camera_count; ++i) {
