...
//-------------------------------------------------------------------------------------------------
Camera::Camera(FrameCallback frame_callback, const std::string& name_hint)
  : impl_(std::make_unique<Impl>()), frame_callback_(std::move(frame_callback)) {
  ...
  if ((cameras_ids == nullptr) or (camera_count == 0)) {
    panic<Exception>(
        std::format("No cameras found, or unable to enumerate cameras: {}", SDL_GetError()));
  }
...
void Camera::acquire() {
  ...
  const auto frame = grape::camera::ImageFrame{
    .header = { .pitch = static_cast<std::uint32_t>(sdl_frame->pitch),
                ...
                .timestamp = now },
    .pixels = { static_cast<std::byte*>(sdl_frame->pixels),
                static_cast<std::uint32_t>(sdl_frame->pitch * sdl_frame->h) }
  };
