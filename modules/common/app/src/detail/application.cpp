...
[[nodiscard]] auto findConfigFile(const std::filesystem::path& file_name)
  ...
  for (const auto& dir : search_dirs) {
    const auto full_path = dir / "config" / file_name;
...
auto configureLogger(const grape::script::ConfigTable& table) -> grape::log::Config {
  ...
  if (not maybe_severity_str) {
    grape::panic<grape::Exception>("severity_threshold not specified");
  }
  const auto maybe_severity = grape::enums::enum_cast<grape::log::Severity>(*maybe_severity_str);
