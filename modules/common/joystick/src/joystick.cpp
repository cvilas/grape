...
//-------------------------------------------------------------------------------------------------
void Joystick::close() {
  const auto was_fd = impl_->device_fd;
  if (impl_->device_fd >= 0) {
  ...
  try {
    if (was_fd != Impl::INVALID_FD) {  // raise disconnect event once per connect event
...
auto Joystick::wait(std::chrono::milliseconds timeout) -> bool {
  ...
  while (const auto bytes_read = ::read(impl_->device_fd, &iev, EVENT_SZ)) {
    if (bytes_read < 0) {
      const auto err = std::error_code(errno, std::system_category());
      if (errno == EAGAIN) {
...
//-------------------------------------------------------------------------------------------------
auto Joystick::readState() -> bool {
  const auto tp = Clock::now();

  // Get mask for available buttons on the device
  ...
  for (auto code = 0U; code < KEY_MAX; ++code) {
    ...
    auto pressed = 0;
    // NOLINTNEXTLINE(cppcoreguidelines-pro-type-vararg)
    if (::ioctl(impl_->device_fd, EVIOCGKEY(code), &pressed) < 0) {
      const auto err = std::error_code(errno, std::system_category());
      impl_->callback(ErrorEvent{ .timestamp = tp,
                                  .message = std::format("Unable to read button '{}': {}",
                                                         toString(control_id), err.message()) });
      return false;
    }

    impl_->callback(ButtonEvent{ .timestamp = tp, .id = control_id, .pressed = (pressed != 0) });
  }

  // Get mask for available axes
