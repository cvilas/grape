...
auto main() -> int {
  ...
  try {
    ...
    if (is_mem_locked.code) {
      std::println("Main thread: {}: {}. Continuing ..", is_mem_locked.function_name,
                   is_mem_locked.code.message());
    }
    ...
    if (is_main_cpu_set.code) {
      std::println("Main thread: {}: {}. Continuing ..", is_main_cpu_set.function_name,
                   is_main_cpu_set.code.message());
    } else {
    ...
    task_config.setup = []() -> bool {
      ...
      if (is_task_cpu_set.code) {
        std::println("Task thread: {}: {}. Continuing ..", is_task_cpu_set.function_name,
                     is_task_cpu_set.code.message());
      } else {
      ...
      if (is_scheduled.code) {
        std::println("Task thread: {}: {}. Continuing ..", is_scheduled.function_name,
