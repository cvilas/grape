...
void ScrollingBuffer::addFrame(std::span<const std::byte> frame) {
  ...
  std::copy(frame.begin(), frame.end(), frame_data_.begin() + offset);
...
Monitor::Monitor() : impl_{ std::make_unique<Impl>() } {
  if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_TIMER) != 0) {
    panic<MonitorException>(std::format("Error: SDL_Init: {}", SDL_GetError()), Error::Renderer);
  }

  // Enable native input method editor.
  // SDL_SetHint(SDL_HINT_IME_SHOW_UI, "1");

  // Create window with graphics context
  static constexpr auto WINDOW_WIDTH = 1280;
  static constexpr auto WINDOW_HEIGHT = 720;
  static constexpr auto WINDOW_FLAGS = SDL_WINDOW_RESIZABLE | SDL_WINDOW_HIDDEN;
  impl_->window = SDL_CreateWindow("Monitor", WINDOW_WIDTH, WINDOW_HEIGHT, WINDOW_FLAGS);
  if (impl_->window == nullptr) {
    panic<MonitorException>(std::format("SDL_CreateWindow: {}", SDL_GetError()), Error::Renderer);
  }
  impl_->renderer = SDL_CreateRenderer(impl_->window, nullptr);
  if (impl_->renderer == nullptr) {
    panic<MonitorException>(std::format("SDL_CreateRenderer: {}", SDL_GetError()), Error::Renderer);
  }
  ...
  if (nullptr == impl_->imgui_ctx) {
    panic<MonitorException>("ImGui::CreateContext", Error::Renderer);
  }
  ...
  if (not ImGui_ImplSDL3_InitForSDLRenderer(impl_->window, impl_->renderer)) {
    ...
    std::ignore = std::fprintf(stderr, "Error: ImGui_ImplSDL3_InitForSDLRenderer\n");
  }
  if (not ImGui_ImplSDLRenderer3_Init(impl_->renderer)) {
    // NOLINTNEXTLINE(cppcoreguidelines-pro-type-vararg)
    std::ignore = std::fprintf(stderr, "Error: ImGui_ImplSDLRenderer3_Init\n");
  }
...
void Monitor::run() {
  ...
  while (is_running) {
    ...
    SDL_SetRenderDrawColor(impl_->renderer, BK_COLOR[0], BK_COLOR[1], BK_COLOR[2], BK_COLOR[3]);
    SDL_RenderClear(impl_->renderer);
    ImGui_ImplSDLRenderer3_RenderDrawData(ImGui::GetDrawData(), impl_->renderer);

    if (0 != SDL_RenderPresent(impl_->renderer)) {
      // NOLINTNEXTLINE(cppcoreguidelines-pro-type-vararg)
      std::ignore = std::fprintf(stderr, "Error: SDL_RenderPresent: %s\n", SDL_GetError());
      is_running = false;
    }
  }
}
