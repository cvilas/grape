...
| `BUILD_CHANGED_MODULES` | `string` | _(none)_ | Git ref to diff against (e.g. `origin/main`). Maps changed files to module names, then builds those modules plus their upstream dependencies and transitive downstream dependees. Intended for CI pipelines on merge requests: pass the destination branch and only the changed slice of the graph is built and tested. |
