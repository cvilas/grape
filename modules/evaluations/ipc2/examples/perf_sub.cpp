...
struct Statistics {
  ...
  void print() const;
  void reset();
  void add(const std::chrono::system_clock::time_point& ts, const grape::ipc2::Sample& sample);
};
...
void Statistics::add(const std::chrono::system_clock::time_point& ts,
                     const grape::ipc2::Sample& sample) {
  if (msg_count == 0) {
    start = ts;
  }
  msg_count++;
...
auto main(int argc, const char* argv[]) -> int {
  try {
    ...
    config.scope = grape::ipc2::Session::Config::Scope::Network;
    auto session = grape::ipc2::Session(config);

    Statistics stats;

    const auto data_cb = [&stats](const grape::ipc2::Sample& sample) {
    ...
    auto sub = session.createSubscriber(grape::ipc2::ex::perf::TOPIC, data_cb);
