...
auto main(int argc, const char* argv[]) -> int {
  try {
    ...
    const auto on_teleop_status = [](const auto& status) {
      std::println("Teleop status: is_active={}, command_latency={}", status.is_active,
                   status.command_latency);
    };
    ...
    while (grape::ipc::ok()) {
      ...
      if(enable){
      teleoperator.send(grape::robot::loco::KeepAliveCmd{});  // No-op; keep teleop alive
      }
