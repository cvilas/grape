...
public:
  ...
  struct Status {
    ...
    std::chrono::system_clock::duration command_latency{};
  };
  ...
  /// @note The command is ignored if teleoperation is not enabled (see enable())
  void send(const loco::TeleopCommandTopic::DataType& cmd);

private:
  void onServerMatch(const ipc::Match& match) const;
  void onLocoStatus(const ipc::Sample& sample) const;
  std::uint64_t id_{ 0 };
  StatusCallback status_cb_{ nullptr };
  grape::ipc::Subscriber loco_status_sub_;
