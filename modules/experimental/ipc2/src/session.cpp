...
//-------------------------------------------------------------------------------------------------
auto transform(const grape::ipc2::Session::Config& config) -> zenoh::Config {
  ...
  if (config.mode == grape::ipc2::Session::Mode::Client and not is_router_specified) {
...
//-------------------------------------------------------------------------------------------------
auto transform(const std::vector<zenoh::Id>& zids) -> std::vector<grape::ipc2::UUID> {
  auto uuids = std::vector<grape::ipc2::UUID>{};
  uuids.reserve(zids.size());

  std::ranges::transform(zids, std::back_inserter(uuids), [](const zenoh::Id& zid) {
    return grape::ipc2::UUID{ .bytes = zid.bytes() };
  });
  return uuids;
}

//-------------------------------------------------------------------------------------------------
auto createDataCallback(grape::ipc2::DataCallback&& user_cb)
