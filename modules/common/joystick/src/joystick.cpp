...
[[nodiscard]] constexpr auto toControlType(unsigned int ev_type) -> grape::joystick::ControlType {
  ...
  switch (ev_type) {
    ...
    default: return ControlType::Unknown; //TODO(vilas): log this
      // clang-format on
  }
}
...
[[nodiscard]] constexpr auto toControlId(unsigned int code) -> grape::joystick::ControlId {
  ...
  switch (code) {
    ...
    default: return ControlId::Unknown; //TODO(vilas): log this
      // clang-format on
  }
}
...
//-------------------------------------------------------------------------------------------------
auto checkBit(unsigned int bit_number, const std::vector<std::uint8_t>& bit_map) -> bool {
  const auto byte_idx = bit_number / 8U;
  const auto bit_idx = bit_number % 8U;
  return ((bit_map.at(byte_idx) & (1U << bit_idx)) != 0);
}
...
private:
  auto readCapabilities() -> std::expected<void, std::string>;
  auto pollForEvents() -> std::expected<void, std::string>;
  auto waitForEvents() -> std::expected<void, std::string>;
...
//-------------------------------------------------------------------------------------------------
auto Joystick::Impl::open() -> std::expected<void, std::string> {
  close();
  ...
  if (fd_ < 0) {
    const auto err = std::error_code(errno, std::system_category());
    return std::unexpected{ std::format("{}: {}", device_path_.c_str(), err.message()) };
  }
  ...
  if (::ioctl(fd_, EVIOCGRAB, 1) != 0) {
    const auto err = std::error_code(errno, std::system_category());
    close();
    return std::unexpected{ std::format("Cannot get exclusive access: {}", err.message()) };
  }

  if (const auto& caps_result = readCapabilities(); not caps_result) {
    close();
...
//-------------------------------------------------------------------------------------------------
auto Joystick::Impl::capabilities() const -> const Capabilities& {
  return capabilities_;
}

//-------------------------------------------------------------------------------------------------
auto Joystick::Impl::readCapabilities() -> std::expected<void, std::string> {
  capabilities_.clear();

  // read all control types on device
  // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers)
  std::vector<std::uint8_t> type_codes_bitmap((EV_MAX + 7U) / 8U, 0U);
  // NOLINTNEXTLINE(cppcoreguidelines-pro-type-vararg)
  ...
  for (const auto ev_type : std::array<unsigned int, 2>{ EV_KEY, EV_ABS }) {
    ...
    if (checkBit(ev_type, type_codes_bitmap)) {
      // read all controls of the type
      // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers)
      std::vector<std::uint8_t> key_codes_bitmap((KEY_MAX + 7U) / 8U, 0U);
      // NOLINTNEXTLINE(cppcoreguidelines-pro-type-vararg)
      ...
      for (unsigned int code = 0; code < KEY_MAX; code++) {
        ...
        if (checkBit(code, key_codes_bitmap)) {
          const auto id = toControlId(code);
          if (id == ControlId::Unknown) {
            std::ignore = std::fprintf(stderr, "Unsupported control ID %x", code);
            continue;
          }
          available_controls.push_back(id);
        }  // if control is available
      }  // for each control of a type
      capabilities_[toControlType(ev_type)] = available_controls;
...
//-------------------------------------------------------------------------------------------------
auto Joystick::Impl::pollForEvents() -> std::expected<void, std::string> {
...
//-------------------------------------------------------------------------------------------------
auto Joystick::Impl::waitForEvents() -> std::expected<void, std::string> {
  // set attempt_reconnection in case of errors and log the error (dont throw)
  return {};
}
...
//-------------------------------------------------------------------------------------------------
auto Joystick::capabilities() const -> const Capabilities& {
