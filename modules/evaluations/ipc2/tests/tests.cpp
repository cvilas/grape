...
TEST_CASE("Basic pub-sub on large message works", "[ipc]") {
  auto session = grape::ipc2::Session(grape::ipc2::Session::Config{});
  const auto topic = grape::ipc2::Topic{ .name = "pub_sub_test" };
  ...
  const auto recv_callback = [&recv_mut, &recv_cond,
                              &received_msg](const grape::ipc2::Sample& sample) {
